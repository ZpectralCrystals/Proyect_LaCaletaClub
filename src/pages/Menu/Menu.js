...
var Menu = function () {
    return (<main className="bg-sky-50 pt-24">

      {/* Íconos de categorías */}
      <div className="flex py-12 justify-center">
        <ul className="flex flex-wrap justify-center gap-8">
          {[
            { icon: free_solid_svg_icons_1.faHotdog, label: 'Desayuno' },
            { icon: free_solid_svg_icons_1.faBurger, label: 'Almuerzo' },
            { icon: free_solid_svg_icons_1.faUtensils, label: 'Cena' },
            { icon: free_solid_svg_icons_1.faCheese, label: 'Postres' },
            { icon: free_solid_svg_icons_1.faWineGlassEmpty, label: 'Bebidas' },
            { icon: free_solid_svg_icons_1.faIceCream, label: 'Helados' },
        ].map(function (item, index) { return (<li key={index} className="text-center relative group">
              <div className="text-xl text-sky-600 group-hover:text-sky-800 relative z-10">
                <react_fontawesome_1.FontAwesomeIcon icon={item.icon}/>
              </div>
              ...
              <div className="absolute left-0 right-0 bottom-[-30px] text-sky-400 group-hover:text-sky-800 group-hover:translate-y-[20px] transition-transform duration-300 ease-in-out z-0">
                <react_fontawesome_1.FontAwesomeIcon icon={free_solid_svg_icons_1.faBookmark} style={{ width: '100%', height: '60px' }}/>
              </div>
            </li>); })}
        </ul>
      </div>
      ...
      <div className="flex flex-col mx-auto py-12 w-[90%] max-w-6xl">
        ...
        <div className="grid md:grid-cols-2 gap-8">
          {[1, 2, 3, 4].map(function (item) { return (<div key={item} className="flex gap-4 group">
              <div className="relative w-32 h-32">
                <span className="absolute top-4 left-0 z-0 w-full h-full rotate-6 bg-sky-100 rounded-lg shadow-inner"></span>
                <img src="/menu.jpg" alt="Plato" className="relative z-10 w-full h-full object-cover rounded-lg border border-sky-200 p-1"/>
              </div>
              <div className="flex-1">
                <div className="flex justify-between items-center mb-2">
                  ...
                  <div className="relative">
                    <react_fontawesome_1.FontAwesomeIcon icon={free_solid_svg_icons_1.faCertificate} className="text-sky-600 group-hover:text-sky-800" style={{ width: '50px', height: '50px' }}/>
